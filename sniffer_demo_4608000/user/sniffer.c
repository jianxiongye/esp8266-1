...
/*	for(leni=0; leni<len;leni++){
    ...
    if (len == 12){
		...
		if(framecontrol->frametype == FRAME_CONTROL)
				return;
...
#define PASS_10 0
		...
		switch(framecontrol->frametype){
			case FRAME_MANAGE://00
				...
				if(framecontrol->subtype == SUBTYPE_PRBREQ \
					|| framecontrol->subtype == SUBTYPE_AUTH\
					|| framecontrol->subtype == SUBTYPE_ASSREQ\
					|| framecontrol->subtype == SUBTYPE_ACTION)//00|0d  00|00
					goto printDS;
				else
					return;
				break;
			case FRAME_CONTROL://01
				//os_printf("FRAME_CONTROL "); //01|09(BlockAck)
			...
			case FRAME_DATA://02  //02|00  
				if( framecontrol->subtype == SUBTYPE_DATA ||\
					framecontrol->subtype == SUBTYPE_NULL_DATA ||\
					framecontrol->subtype == SUBTYPE_QOS_DATA ||\
					framecontrol->subtype == SUBTYPE_QOS_NULL_DATA)
				{
						//ap发出的帧,可以用来当作探针数据
						if(framecontrol->datadir == DATA_DIR_FROMAP)
							//ap广播，我们需要将其过滤
		...
		//打印本机stamac
  			// 缓存上次的MAC，避免重复打印
printDS:if(0!=sniffer->buf[10]%4)//如果不能被4整除除，那就不是我们需要的mac
			return;
		// 如果MAC地址和上一次一样就返回
	...
	return;
		 		...
		 		// 缓存上次的MAC，避免重复打印
printSD:if(0!=sniffer->buf[4]%4)//如果不能被4整除除，那就不是我们需要的mac
			return;
