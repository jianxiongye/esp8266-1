...
/*	for(leni=0; leni<len;leni++){
  ...
  //  os_printf("\ncoming count =%ld c12=%ld c128=%ld celse=%d len-%d\r\n",count++,c12,c128,celse,len);
	if(len<12){
		os_printf("!!!!error!!!! len=%d\r\n",len);
		return;
	}
	struct framecontrol_t *framecontrol = (struct framecontrol_t*)(&buf[12]);
	
	//过滤掉beacon帧
	if(framecontrol->subtype != 4)
			return;
    ...
    if (len == 12){
		...
		if (len == 128) {
			...
			//os_printf("mac,rssi,channel");
			//printmac(sniffer->buf, 4);
			//os_printf("|");
			printmac(sniffer->buf, 10);
			//os_printf("|");
			//printmac(sniffer->buf, 16);
			os_printf(",%3d,%2d,", sniffer->rx_ctrl.rssi, sniffer->rx_ctrl.channel);

			os_printf("\n");
			return;
			
			if(0==os_memcmp(client, &sniffer->buf[10], 6)){
				os_printf("detected asdazaki:%s\r\n",buf);
				for(leni=0; leni<len;leni++){
						os_printf("0x%x ", buf[leni]);
			...
			os_printf("|");
			printmac(sniffer->buf, 16);
			os_printf("|rssi:%3d |channel:%2d", sniffer->rx_ctrl.rssi, sniffer->rx_ctrl.channel);
			os_printf("\n");
			return;
