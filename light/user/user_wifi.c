...
wifi_handle_event_cb(System_Event_t *evt) {
	...
	case EVENT_STAMODE_DISCONNECTED:
		...
		/*if(evt->event_info.disconnected.reason == 201){
			...
			user_set_station_config(WIFI_SSID_WAYZ, WIFI_PASSWORD_WAYZ);
			wifi_set_event_handler_cb(wifi_handle_event_cb);
			wifi_station_connect();
		}else if(evt->event_info.disconnected.reason == 2){
			wifi_station_connect();
			os_printf("connecet again\n");

		}*/
static u16 count_disconnected =0;
		count_disconnected ++;
		if(count_disconnected > 20)
			system_restart();
		break;
