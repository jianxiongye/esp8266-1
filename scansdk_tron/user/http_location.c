...
scan_done(void *arg, STATUS status) {
	if (status == OK) {
		...
		if(wificount ==0){
			...
			os_printf("only one wifi found!!!\r\n");
			os_sprintf(buf2, POST_WAYZ_LO, buf);


			//合并出content内容
			os_sprintf(buf,POST_WAYZ_INFO, time, chip_id, MAC2STR(macAddr), MAC2STR(macAddr), buf2);
			//os_sprintf(buf3,POST_WAYZ_INFO, time, chip_id, MAC2STR(macAddr), MAC2STR(macAddr));
			//os_sprintf(buf2,POST_WAYZ_CONTENT, buf3, buf);

			//合并出http数据
			os_sprintf(buf2,POST_WAYZ_SSL, os_strlen(buf), buf);
			os_printf("wificount2=%d buf=\r\n===========\r\n%s\r\n==========\r\n",wificount, buf2);
		}else {
			os_sprintf(buf2, POST_WAYZ_WIFIS, buf);
			os_sprintf(buf, POST_WAYZ_LO, buf2);


			//合并出content内容
			os_sprintf(buf2,POST_WAYZ_INFO, time, chip_id, MAC2STR(macAddr), MAC2STR(macAddr), buf);
			//os_sprintf(buf3,POST_WAYZ_INFO, time, chip_id, MAC2STR(macAddr), MAC2STR(macAddr));
			//os_sprintf(buf2,POST_WAYZ_CONTENT, buf3, buf);

			//合并出http数据
			os_sprintf(buf,POST_WAYZ_SSL, os_strlen(buf2), buf2);
			os_printf("wificount2=%d buf=\r\n===========\r\n%s\r\n==========\r\n",wificount, buf);
		}
		dns_test();
