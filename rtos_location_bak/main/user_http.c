...
/* 存放get请求的相关数据 */
static uint8_t https_get_buffer[1024 * 2];
/* 存放接收到GET请求的数据 */
...
static void https_get_reuest_json_data_task(void *pvParameters)
{
  ...
  https_get_reuest_json_data_parse(cJSON_Parse(strchr(json_data, '{')));  
  free(json_data);
  vTaskDelete(NULL);
}
...
{
  int ret, len;  
  recv_json_buff = (char*)malloc(sizeof(char)*1280);
  mbedtls_net_context *net_ctx = (mbedtls_net_context *)malloc(sizeof(mbedtls_net_context));
  ...
  mbedtls_ssl_context *ssl_ctx = (mbedtls_ssl_context *)malloc(sizeof(mbedtls_ssl_context));
  ...
  {
    len = sprintf((char*)https_get_buffer, POST, filename, strlen(POST_CONTENT), host, cJsonBuffer);
  }
  ...
  do
  {
    ...
    {
      len = ret;
      memcpy(recv_json_buff,recv_buff,len);
