...
tcp_client_discon_cb(void *arg) {
	...
	//os_printf("tcp_client_discon_cb reconnect\n");
}
...
tcp_client_recv(void *arg, char *pdata, unsigned short len) {
	...
	os_printf("data:%s\r\n", pdata);
	char *pBody = NULL;
	char dir_open1[4]  = {0xA0,0x01,0x01,0xA2};
	char dir_close1[4] = {0xA0,0x01,0x00,0xA1};




	get_http_body(pdata, &pBody);
	if(pBody[0] == 'C')
	{diarmTimer();
...
http_get_lightstate_init(u8* ip, u16 port) {
...
void ICACHE_FLASH_ATTR
http_get_lightstate_connect(void) {
	s8 rc = 0;
...
void ICACHE_FLASH_ATTR
http_get_lightstate_send(void) {
	//espconn_send((struct espconn *)g_ptcp_conn->proto.tcp, HTTP_REQUEST, os_strlen(HTTP_REQUEST));
	http_get_lightstate_connect();
}
