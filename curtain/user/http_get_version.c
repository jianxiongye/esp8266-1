...
tcp_client_recv(void *arg, char *pdata, unsigned short len) {
	...
	debug("[getversion]tcp_client_recv\n");
//	debug("lenth:%d\r\n", len);
	debug("data:\r\n%s\r\n", pdata);
...
// 获取http body size
	if (http_body_size == 0) {
		...
		//如果一次就接受完毕
		if(!otaPassed && pBody != NULL && pBody[0]>'0'){
			otaPassed =1;
			//判断version
			os_printf("ALL zaki verion ===========pBody=%s \r\n",pBody);
			
			os_printf("cloud verion=%s, local version=%c.%c.%c \r\n",pBody, MAJOR_VERSION, MINOR_VERSION, REVERSION_VERSION);
			if(MAJOR_VERSION < pBody[0])
				;
			else if(MINOR_VERSION < pBody[2])
				;
			else if(REVERSION_VERSION < pBody[4])
				;
			else{
				task_postb();
				return;
		...
		if(!otaPassed && pBody != NULL && pBody[0]>'0'){
			otaPassed =1;
			//判断version
			os_printf("More zaki verion ===========pBody=%s \r\n",pBody);
...
espconn_send(pesp_conn, HTTP_VERSION, os_strlen(HTTP_VERSION));
...
static void ICACHE_FLASH_ATTR
tcp_client_reconnect_cb(void *arg, sint8 err) {
	debug("tcp_client_reconnect_cb err=%d\r\n", err);
...
http_get_version_init(char* ip, u16 port) {
...
espconn_regist_reconcb(g_ptcp_conn,tcp_client_reconnect_cb);
