...
scan_done(void *arg, STATUS status) {
	if (status == OK) {
		...
		{	wificount++;
			...
			bss_link= bss_link->next.stqe_next;
			os_printf("%s\r\n",pbuf);
			//最多接受40个wifi扫描数据，解决wifi扫描个数多导致buf溢出问题
			if(wificount>=40)
				break;

		}
		
		wifiscanLen=os_strlen(buf);


		//os_printf("buf=%s\n",buf);
		spi_flash_erase_sector(secD_addr/FourKB);
		spi_flash_write(secD_addr, (uint32 *)buf,         wifiscanLen);
		//spi_flash_read(secD_addr, (uint32 *)des_addr,     wifiscanLen);
		//os_printf("des_addr=%s\n",des_addr);
