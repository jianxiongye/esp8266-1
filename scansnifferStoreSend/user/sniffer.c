...
//发送时间，默认值为10min
extern u16 m_sniff_send_loop_min;
...
channelHop(void *arg)
{
   ...
   static uint8 new_channel = 1;
	...
	if(m_min>=m_sniff_send_loop_min){
		os_printf("time is up 11min send data now\n");
		m_min=0;
		m_count=0;
		task_post('d');
	}
			...
			case FRAME_DATA://02  //02|00  
				{
						...
						if(framecontrol->datadir == DATA_DIR_FROMAP){
							if(!os_memcmp(&sniffer->buf[10], &sniffer->buf[16], 6))
								goto printSD;
							else
								goto print32;
...
*/		os_sprintf(mstore,"%02X:%02X:%02X:%02X:%02X:%02X|%02X:%02X:%02X:%02X:%02X:%02X|%02d|%02d|%02d|%d|0|%d|%d\n",\
		PRINT(sniffer->buf,10),PRINT(sniffer->buf,4),\
		framecontrol->frametype, framecontrol->subtype, sniffer->rx_ctrl.channel, sniffer->rx_ctrl.rssi,framecontrol->datadir,m_min);
		os_printf(mstore);
...
printSD:
	...
	/*	os_printf("%02X:%02X:%02X:%02X:%02X:%02X|%02X:%02X:%02X:%02X:%02X:%02X|%02X:%02X:%02X:%02X:%02X:%02X|%02d|%02d|%d|%d|0|%d|0\n",\
		...
		*/
		...
		return;
print32:
		if(0==os_memcmp(temp_mac1, &sniffer->buf[16], 6)){
			return;
		}
		for (i=0; i<6; i++){
			temp_mac1[i] = sniffer->buf[i+16];
		}
	/*	os_printf("%02X:%02X:%02X:%02X:%02X:%02X|%02X:%02X:%02X:%02X:%02X:%02X|%02X:%02X:%02X:%02X:%02X:%02X|%02d|%02d|%d|%d|0|%d|0\n",\
		PRINT(staMacAddr,0),PRINT(sniffer->buf,4),PRINT(sniffer->buf,10),\
		framecontrol->frametype, framecontrol->subtype, sniffer->rx_ctrl.channel, sniffer->rx_ctrl.rssi,framecontrol->datadir);
		*/
		...
		os_sprintf(mstore,"%02X:%02X:%02X:%02X:%02X:%02X|%02X:%02X:%02X:%02X:%02X:%02X|%02d|%02d|%02d|%d|0|%d|%d\n",\
				PRINT(sniffer->buf,16),PRINT(sniffer->buf,10),\
				framecontrol->frametype, framecontrol->subtype, sniffer->rx_ctrl.channel, sniffer->rx_ctrl.rssi,framecontrol->datadir,m_min);
