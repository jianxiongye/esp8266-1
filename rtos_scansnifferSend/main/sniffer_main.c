...
#if 1
    int i = 0;
    //char aucMacBroadCast[] = "C8:2B:96:08:61:FD"; // 广播的mac地址
	//char aucMacMultiCast[] = "CA:2B:96:08:61:FD"; // 多播的mac地址
...
void channelHopFunc(void *arg)
{
	...
	if(tInfoMisc.count % TIMEOUT_COUNT == 0){
		...
		updataScanTime++;
		if (updataScanTime >= SCANTIME)
		{
			tInfoMisc.scanFlag = ENBLE;
			updataScanTime = 0;
		}
	}
