            ...
            switch (event.type) {
                ...
                case UART_DATA:
                    ...
                    if (0 == strncmp(dtmp, "set", 3))
                    {
                        ...
                        {
                            case 'N': // wifi 名字
                                memset(tInfoMisc.tuartWifiInfo.mWifiName, 0, sizeof (tInfoMisc.tuartWifiInfo.mWifiName));
                                memcpy(tInfoMisc.tuartWifiInfo.mWifiName, dtmp + 5, event.size - 5);
                                ESP_LOGI(TAG, "wifi name: %s", tInfoMisc.tuartWifiInfo.mWifiName);
                                NVS_Write(WIFI_NAME_KEY, tInfoMisc.tuartWifiInfo.mWifiName, sizeof (tInfoMisc.tuartWifiInfo.mWifiName));
                                ESP_ERROR_CHECK(esp_wifi_set_promiscuous(false));
                                break;
                            case 'P': // wifi 密码
                                memset(tInfoMisc.tuartWifiInfo.mWifiPass, 0, sizeof (tInfoMisc.tuartWifiInfo.mWifiPass));
                                memcpy(tInfoMisc.tuartWifiInfo.mWifiPass, dtmp + 5, event.size - 5);
                                ESP_LOGI(TAG, "wifi passwd: %s", tInfoMisc.tuartWifiInfo.mWifiPass);
                                NVS_Write(WIFI_PASSWD_KEY, tInfoMisc.tuartWifiInfo.mWifiPass, sizeof (tInfoMisc.tuartWifiInfo.mWifiPass));
                                wifi_init_sta();
                                break;
                            case 'S': // 服务器ip + port
                                ret = strstr(dtmp + 5, ":");
                                memset(tInfoMisc.tuartWifiInfo.mServer, 0, sizeof (tInfoMisc.tuartWifiInfo.mServer));
                            ...
                            case 'T': // 设置多长时间上传
                                ...
                                if (tInfoMisc.tuartWifiInfo.mSniffSendMin >= (SNIFFER_MAX_TIME / 2))
                                {
                                    tInfoMisc.fifoFlag = ENBLE;
                                }
                                else
                                {
                                    tInfoMisc.fifoFlag = DISABLE;
                                }
